package com.nikhil.SaavnProject;
...
public class SaavnFilter
{
  ...
  public Configuration getConf() {
	...
	public static class ReducerClass extends Reducer<Text, DateAndCount, Text, Text> 
	{
		...
		{	
   			int song_count=0;
   			for(int date=24;date<=31;date++)
   		{
   			for (DateAndCount var : valueList) 
			{	
   				song_count+=var.getCount()/(date-Integer.parseInt(var.getDate()));
   			}
   			TreeMap<Integer,String> tempCollection = new TreeMap<Integer,String>();
   			NavigableMap<Integer,String> songIdnCount= tempCollection.descendingMap();
   			songIdnCount.put(song_count,key.toString());
   			while(songIdnCount.size() > 100){
   	   			if(songIdnCount.lastKey() < song_count ){
   	   			songIdnCount.remove(songIdnCount.lastKey());
   	   			songIdnCount.put(song_count, key.toString());
   	   			}

   			}
   			String out = date+","+song_count;
